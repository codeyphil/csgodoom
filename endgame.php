...
$comitems=array();
$mcom=3; //max items for comission (1-3 recommended)
$ctaken=0;
if($comvalue>0){
	for($i=$itemscount-1; $i>=0; $i--){ //get comission items
		...
		if($itemsarr[$i]['userid']!=$winner['userid']){ //don't take shit from the winner bro
			...
			if($itemsarr[$i]['value']<0.05){ //end the loop if we get to items cheaper than 5 cents, no reason to take that shit
				$i=-1;
			}else{
				if( ( $csofar + $itemsarr[$i]['value'] ) <= $comvalue && $itemsarr[$i]['value']>0.05){
...
$cg++;
$winnerpercent = mt_rand(0,99).'.'.mt_rand(00000001,99999999); //winner percent
